...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const parlamentaresPath = path.join(
      process.cwd(),
      "parlamentares_cache.json"
    );
    const parlamentaresData = await fs.readFile(parlamentaresPath, "utf8");
    const parlamentaresCache = JSON.parse(parlamentaresData);

    // Encontrar o parlamentar para adicionar partido e UF
    const parlamentarInfo = parlamentaresCache.parlamentares.find(
      (p) => p.nome.toLowerCase() === novaManifestacao.parlamentar.toLowerCase()
    );
    ...
    const manifestacoesPath = path.join(
      process.cwd(),
      "_data",
      "manifestacoes.yml"
    );
